...
class _NotificationUpdateForm extends HookWidget {
  ...
  Widget build(BuildContext context) {
    final notifTimeCtrl = useTextEditingController(
        text: !kIsWeb
            ? SettingsDataStore.scheduledNotifTime.format(context)
            : '');

    Future<void> updateNotifTime() async {
      final timeOfDay = await showTimePicker(
          context: context, initialTime: SettingsDataStore.scheduledNotifTime);
      if (timeOfDay != null) {
        onLoading();
        await NotificationService.scheduleNotifs(timeOfDay);
        notifTimeCtrl.text =
            SettingsDataStore.scheduledNotifTime.format(context);
        onComplete();
      }
